...
def get_labels(df, end_date: str):
    ...
    df_labels['dt_iso'] = pd.to_datetime(df_labels['dt_iso'], format='%Y-%m-%d %H:%M:%S %z UTC', utc=True)
    df_labels = df_labels[((df_labels['dt_iso'] < out_res['last_changed'].max()) & (df_labels['dt_iso'] > out_res['last_changed'].min()))]
    ...
    out_res_labelled = pd.merge(out_res, df_labels, on='last_changed', how='outer').sort_values(by='last_changed')
